...
public class AppBar extends JPanel {
          ...
          invokeLater(
              () ->
                  new CasSetAnalyzer(DISPOSE_ON_CLOSE)
                      .accept(transpileToutCas(FileSideBar.getCasSetFile().getAbsolutePath())));
        });
  ...
  private void syncSelectedFileWithDrive(JFrame owner) {
    ...
    if (optFile.isEmpty()) {
      showMessageDialog(
          this, "Aucun fichier local sélectionné.", "Erreur", JOptionPane.ERROR_MESSAGE);
      return;
    }
    if (optDriveId.isEmpty()) {
      showMessageDialog(
          this,
          "Ce fichier n'est pas lié à un fichier Drive.",
          "Erreur",
          JOptionPane.ERROR_MESSAGE);
      return;
    }
