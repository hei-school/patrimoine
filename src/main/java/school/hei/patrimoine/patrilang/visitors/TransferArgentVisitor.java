...
public class TransferArgentVisitor implements PossessionVisitor<TransfertArgent, FluxArgentTransfererContext> {
    private final Function<String, Compte> compteGetter;

    @Override
    public TransfertArgent visit(FluxArgentTransfererContext ctx) {
        String id = parseNodeValue(ctx.ID());
        LocalDate t = visitDate(ctx.date());
        Argent valeurComptable = visitArgent(ctx.argent());
        Compte compteDepuis = compteGetter.apply(parseNodeValue(ctx.TEXT(0)));
        Compte compteVers  = compteGetter.apply(parseNodeValue(ctx.TEXT(1)));
