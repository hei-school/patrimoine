...
public class PossessionReoupeeDetailDialog extends Dialog {
  ...
  private void addMainTitle() {
    var titleLabel =
        new JLabel(
            String.format(
                "<html><b>Détails de l'opération: %s</b></html>",
  ...
  private void addInfoPanel() {
    ...
    infoPanel.add(makeInfoRow("Date prévue", formatDate(possessionRecoupee.datePrevu())));
    infoPanel.add(makeInfoRow("Date réalisée", formatDate(possessionRecoupee.dateRealise())));
    infoPanel.add(makeInfoRow("Valeur prévue", formatArgent(possessionRecoupee.valeurPrevu())));
    infoPanel.add(makeInfoRow("Valeur réalisée", formatArgent(possessionRecoupee.valeurRealise())));
    infoPanel.add(
        makeInfoRow("Écart réalisé", formatArgent(possessionRecoupee.valeurDifference())));
    infoPanel.add(
        makeInfoRow(
            "Jours de différence", String.valueOf(possessionRecoupee.dateDifferenceEnJour())));

    add(infoPanel, BorderLayout.WEST);
  }

  static JPanel makeInfoRow(String labelText, String valueText) {
