...
class TrainDeVieTest {
    ...
    void train_de_vie_est_finance_par_compte_courant() {
        ...
        var vieEstudiantine = new TrainDeVie(
                ...
                compteCourant,
                1);
        var MoisApresHEIOuvert = Instant.parse("2021-11-26T00:00:00.00Z");
        var trainDeVie1MoisApresHEIOuvert = (TrainDeVie) vieEstudiantine.projectionFuture(MoisApresHEIOuvert);
      assertEquals(
              100_000,
              trainDeVie1MoisApresHEIOuvert.getFinancePar().getValeurComptable());
        assertEquals(vieEstudiantine.getDepensesMensuelle(), trainDeVie1MoisApresHEIOuvert.getDepensesMensuelle());
        var auDiplomation = Instant.parse("2025-12-26T00:00:00.00Z");
        assertEquals(0,
                vieEstudiantine.projectionFuture(auDiplomation).getDepensesMensuelle());
    }
