...
public class CommentSideBar extends JPanel {
  ...
  private boolean hasSelectedFile() {
    return !isNull(currentFileId) && !currentFileId.isEmpty();
  }
  ...
  public void update(String currentFileId) {
        ...
        new SwingWorker<>() {
          ...
          protected void done() {
            ...
            try {
              ...
              if(comments.isEmpty()){
                var label = new JLabel("Aucun commentaire", SwingConstants.CENTER);
                ...
                for (var comment : comments) {
                    ...
                    cardWrapper.setMaximumSize(new Dimension(Integer.MAX_VALUE, card.getPreferredSize().height));

                    listPanel.add(cardWrapper);
                    listPanel.add(Box.createVerticalStrut(10));
                }

                var scroll = new JScrollPane(listPanel, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
                scroll.getVerticalScrollBar().setUnitIncrement(16);
                add(scroll, BorderLayout.CENTER);
            } catch (Exception e) {
              showMessageDialog(owner, "Erreur lors de get comment : " + e.getMessage(), "Erreur", JOptionPane.ERROR_MESSAGE);
            }
    ...
    private JPanel renderComment(Comment comment) {
        ...
        var resolveBtn = new Button("Resolve");
