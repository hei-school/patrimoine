...
public class RecoupementPage extends Page {
    ...
    private void init(){
        ...
        possessionsFilterSelect.addActionListener(e -> {
            state = new State(fileList.getSelectedValue(), (Status) possessionsFilterSelect.getSelectedItem());
            onStateChange();
        });

        this.appBar = new AppBar(this::onStateChange, AppBar
            .Conf
            .builder()
                ...
                .customComponents(List.of(new NavigateButton("Retour", "patrilang-files"), possessionsFilterSelect))
            .build()
        );

        this.fileList = new JList<>(new FileListModel(FileSideBar.getPlannedPatrilangFilesWithoutCasSet()));
    ...
    private List<PossessionRecoupée> getFilteredPossessionRecoupées(){
        ...
        switch(state.currentFilterStatus){
            case TOUT -> {
                possessionRecouppées.addAll(toPossessionRecoupées(recoupeurDePossession.getPossessionsNonPrévus(), Status.NON_PRÉVU));
                possessionRecouppées.addAll(toPossessionRecoupées(recoupeurDePossession.getPossessionsNonExecutés(), Status.NON_ÉXECUTÉ));
                possessionRecouppées.addAll(toPossessionRecoupées(recoupeurDePossession.getPossessionsÉxecutésAvecDifférences(), Status.ÉXECUTÉ_AVEC_DIFFÉRENCE));
                possessionRecouppées.addAll(toPossessionRecoupées(recoupeurDePossession.getPossessionsÉxecutésSansDifférences(), Status.ÉXECUTÉ_SANS_DIFFÉRENCE));
            }

            case NON_PRÉVU -> possessionRecouppées.addAll(toPossessionRecoupées(recoupeurDePossession.getPossessionsNonPrévus(), Status.NON_PRÉVU));
            case NON_ÉXECUTÉ -> possessionRecouppées.addAll(toPossessionRecoupées(recoupeurDePossession.getPossessionsNonExecutés(), Status.NON_ÉXECUTÉ));
            case ÉXECUTÉ_AVEC_DIFFÉRENCE -> possessionRecouppées.addAll(toPossessionRecoupées(recoupeurDePossession.getPossessionsÉxecutésAvecDifférences(), Status.ÉXECUTÉ_AVEC_DIFFÉRENCE));
            case ÉXECUTÉ_SANS_DIFFÉRENCE -> possessionRecouppées.addAll(toPossessionRecoupées(recoupeurDePossession.getPossessionsÉxecutésSansDifférences(), Status.ÉXECUTÉ_SANS_DIFFÉRENCE));
        }

        return possessionRecouppées.stream().sorted(Comparator.comparing((PossessionRecoupée p) -> p.possession().t()).reversed()).toList();
    }
    ...
    private static Set<PossessionRecoupée> toPossessionRecoupées(Set<Possession> possessions, Status status){
