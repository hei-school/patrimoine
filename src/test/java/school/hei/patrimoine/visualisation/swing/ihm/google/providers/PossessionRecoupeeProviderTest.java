...
class PossessionRecoupeeProviderTest {
    ...
    void getList_empty_data() {
        ...
        var cas = new Cas(date, date, Map.of()) {
            @Override
    ...
    void getList_with_status_filter() {
        ...
        var cas = new Cas(date, date, Map.of()) {
            ...
            @Override protected void suivi() {}
            @Override public Set<Possession> possessions() { return Set.of(compte); }
            @Override protected Devise devise() { return Devise.MGA; }
            @Override protected String nom() { return "patrimoine"; }
        };
        ...
        assertTrue(result.possessionRecoupees().stream().allMatch(p -> p.status() == RecoupementStatus.EXECUTE_SANS_CORRECTION));
    }
    ...
    void getList_with_name_filter() {
        ...
        var cas = new Cas(date, date, Map.of()) {
            @Override protected void init() {}
            @Override protected void suivi() {}
            @Override public Set<school.hei.patrimoine.modele.possession.Possession> possessions() { return Set.of(compte); }
            @Override protected Devise devise() { return Devise.MGA; }
            @Override protected String nom() { return "patrimoine"; }
        };
    ...
    void getList_pagination_total_pages() {
        ...
        var cas = new Cas(date, date, Map.of()) {
            @Override protected void init() {}
            @Override protected void suivi() {}
            @Override public Set<Possession> possessions() { return Set.of(compte1, compte2); }
            @Override protected Devise devise() { return Devise.MGA; }
            @Override protected String nom() { return "patrimoine"; }
        };
